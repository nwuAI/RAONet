...
def evaluate(data_loader, model, device, task, epoch, mode, num_class):
    ...
    print(f'原代码准确率计算为：{acc:.4f}')
    accuracy1 = accuracy_score(true_label_decode_list, prediction_decode_list)
    auc_roc = roc_auc_score(true_label_onehot_list, prediction_list,multi_class='ovr',average='macro')
    auc_pr = average_precision_score(true_label_onehot_list, prediction_list,average='macro')          
            
    metric_logger.synchronize_between_processes()
    
    print('Sklearn Metrics - Acc: {:.4f} AUC-roc: {:.4f} AUC-pr: {:.4f} F1-score: {:.4f} MCC: {:.4f}'.format(accuracy1, auc_roc, auc_pr, F1, mcc)) 
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()},auc_roc,accuracy1,true_label_decode_list,prediction_decode_list
