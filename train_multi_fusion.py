...
from util.misc import plot_confusion_matrix


def parse_args():

    parser = argparse.ArgumentParser(description='CFP-OCT train')
    ...
    parser.add_argument('--resnet_oct_checkpoint', type=str,
                        default='',
                        help='OCT ResNet checkpoint path')
...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        running_loss = 0.0
        running_corrects = 0
        for cfp_image, oct_image, label, _ in train_loader:  # 解包四个返回值，但只用前两个和标签
            cfp_image = cfp_image.to(args.device)
            oct_image = oct_image.to(args.device)
            labels = label.to(args.device)

            optimizer.zero_grad()

            with torch.set_grad_enabled(True):
                outputs = fusion_model(cfp_image, oct_image)
        ...
        auc_roc, acc, all_labels, pred_labels, val_loss = validate_multi_auc_accuracy(fusion_model, val_loader, args.device, args.num_classes, criterion)
    ...
    file_path = os.path.join(args.save_dir, f'cfp_oct_{best_epoch}_{best_acc:.4f}_{best_auc:.4f}.pth')
